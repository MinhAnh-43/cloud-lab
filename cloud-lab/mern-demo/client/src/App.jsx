...
const getApiUrl = () => {
  ...
  if (hostname.includes('app.github.dev')) {
    return `${window.location.protocol}//${hostname.replace(/-3000(?=\.app\.github\.dev)/, '-5000')}/api/students`;
  }

  return `http://${hostname}:5000/api/students`;
};
...
function App() {
  ...
  const [editingId, setEditingId] = useState(null); // ID của sinh viên đang sửa
  ...
  // Gửi dữ liệu tạo hoặc cập nhật sinh viên
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa sinh viên "${name}" không?`)) return;
    try {
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      <div className="container mx-auto px-4 py-10 max-w-5xl">
        ...
        <div className="text-center mb-12">
          <h1 className="mt-3 text-2xl font-bold text-gray-900" style={{ color: '#111827' }}>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 sticky top-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full flex justify-center items-center gap-2 px-6 py-3 text-white font-bold rounded-lg transition duration-150 ${loading ? 'bg-gray-400 cursor-not-allowed' : editingId ? 'bg-yellow-500 hover:bg-yellow-600 shadow-md hover:shadow-lg' : 'bg-blue-600 hover:bg-blue-700 shadow-md hover:shadow-lg'}`}
                >
                  {loading ? (
                    <>
                ...
                {editingId && (
                  <button
                    ...
                    className="w-full px-6 py-3 text-gray-700 font-bold rounded-lg border border-gray-300 hover:bg-gray-100 transition duration-150"
                  >
                    ❌ Hủy Chỉnh Sửa
                  </button>
                )}
                ...
                <table className="w-full text-left table-auto">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider text-center">Hành Động</th>
                    </tr>
                  </thead>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <div className="flex justify-center gap-2">
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-yellow-700 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition"
                                title="Sửa sinh viên"
                              >
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-red-700 bg-red-100 rounded-lg hover:bg-red-200 transition"
                                title="Xóa sinh viên"
                              >
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
